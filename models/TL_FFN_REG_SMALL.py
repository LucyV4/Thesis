...
def get_identifier(triplet_label: str, l2: float, dropout: float, lr: float, margin: float, batch_size: int, n_neighbours: int, min_dist: float):
    return f"TL_FFN_REG_SMALL_{triplet_label}_{l2}L2_{dropout}D_{lr}LR_{margin}Margin_{batch_size}BatchSize_{n_neighbours}Neighbours_{min_dist}MinDist"
...
def train(dataloader: DataLoader, device: str, options: dict) -> list:
	...
	identifier = f"TL_FFN_REG_SMALL_{triplet_label}_{l2}L2_{dropout}D_{lr}LR_{margin}Margin_{batch_size}BatchSize_{n_neighbours}Neighbours_{min_dist}MinDist"
...
def visualize(dataloader: DataLoader, device: str, options: dict):
	...
	for epoch in tqdm(range(n_epoch), desc="Vis epoch"):
		...
		visualizer.UMAP_visualize(
			X_train,
			y_train,
			f"UMAP embedding of train representation - Epoch {epoch+1}",
		...
		visualizer.UMAP_visualize(
			...
			subtitle_str=f"TL-FFN-REG-SMALL params: Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}\nUMAP params: N-neighbours={n_neighbours} - Min distance={min_dist}"
		)
		...
		visualizer.visualize_small_model(
			X_train,
			y_train,
			f"Train representation visualization - Epoch {epoch+1}",
			f"model_figures/{identifier}/TL_FFN_umap_epoch{epoch+1}_train_direct.svg",
			subtitle_str=f"TL-FFN-REG params: Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}"
		)
		...
		visualizer.visualize_small_model(
			X_test,
			y_test,
			f"Test representation visualization - Epoch {epoch+1}",
			f"model_figures/{identifier}/TL_FFN_umap_epoch{epoch+1}_test_direct.svg",
			subtitle_str=f"TL-FFN-REG-SMALL params: Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}"
		)
		...
		visualizer.compare_visualize(
			X_train,
			y_train,
			f"Comparison of training data UMAP embeddings: raw vs epoch {epoch+1}",
			f"model_figures/{identifier}/TL_FFN_umap_epoch{epoch+1}_comp_train.svg",
			fig_subtitle=f"TL-FFN-REG-SMALL params: Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}\nUMAP params: N-neighbours={n_neighbours} - Min distance={min_dist}"
		)
  
		visualizer.compare_visualize(
			X_test,
			y_test,
			f"Comparison of test data UMAP embeddings: raw vs epoch {epoch+1}",
		...
		visualizer.overlay(
			...
			X_test,
			y_test,
			f"Overlay UMAP embedded data at epoch {epoch+1}",
		...
		visualize_loss(
			...
			f"TL-FFN-REG-SMALL - Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}"
		)