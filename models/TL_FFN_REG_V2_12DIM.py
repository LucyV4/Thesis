...
def get_identifier(triplet_label: str, l2: float, dropout: float, lr: float, margin: float, batch_size: int, n_neighbours: int, min_dist: float):
    return f"TL_FFN_REG_V2_12DIM_{triplet_label}_{l2}L2_{dropout}D_{lr}LR_{margin}Margin_{batch_size}BatchSize_{n_neighbours}Neighbours_{min_dist}MinDist"
...
def train(dataloader: DataLoader, device: str, options: dict) -> list:
	...
	identifier = f"TL_FFN_REG_V2_12DIM_{triplet_label}_{l2}L2_{dropout}D_{lr}LR_{margin}Margin_{batch_size}BatchSize_{n_neighbours}Neighbours_{min_dist}MinDist"
	...
	for epoch in range(n_epoch):
		...
		visualize_loss(
			...
			f"TL-FFN-REG V2 12DIM - Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}"
		)
...
def visualize(dataloader: DataLoader, device: str, options: dict):
	...
	for epoch in tqdm(range(n_epoch), desc="Vis epoch"):
		...
		visualizer.UMAP_visualize(
			...
			subtitle_str=f"TL-FFN-REG-V2-12DIM params: Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}\nUMAP params: N-neighbours={n_neighbours} - Min distance={min_dist}"
		)
		visualizer.UMAP_visualize(
			X_test,
			y_test,
		...
		visualizer.compare_visualize(
			...
			fig_subtitle=f"TL-FFN-REG-V2-12DIM params: Label={triplet_label} - LR={lr} - Triplet margin={margin} - L2={l2} - Dropout={dropout}\nUMAP params: N-neighbours={n_neighbours} - Min distance={min_dist}"
		)
